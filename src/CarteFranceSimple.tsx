...
const CarteFranceSimple = () => {
  ...
  const [hoveredDep, setHoveredDep] = useState<any>(null);
  ...
  useEffect(() => {
    const loadSvg = async () => {
      try {
        const response = await fetch('/departements.svg');
          ...
          <div className="mb-4">
            {svgContent && (
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: svgContent.replace(/width="907" height="1000"/, 'width="650" height="600" viewBox="0 0 907 1000"') }}
